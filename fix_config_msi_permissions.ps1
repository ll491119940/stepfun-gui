...
if (-not $isAdmin) {
    ...
    Write-Host ""
    Write-Host "或者，你可以尝试以下方法：" -ForegroundColor Cyan
...
Write-Host "正在检查 $configMsiPath ..." -ForegroundColor Yellow

if (-not (Test-Path $configMsiPath)) {
    Write-Host "Config.Msi 文件夹不存在，可能不需要修复。" -ForegroundColor Green
    exit 0
}
...
try {
    ...
    try {
        ...
        Write-Host "无法读取文件夹权限，尝试使用 takeown 命令..." -ForegroundColor Yellow
